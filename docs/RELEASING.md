...
```text
apix 0.6.0 (commit abc1234, built 2026-06-14T12:00:00Z)
```
...
3. Choose a semantic version such as `v0.6.0`.
4. Create and push an annotated tag:

```bash
git switch main
git pull --ff-only
git tag -a v0.6.0 -m "API Explorer v0.6.0"
...
```bash
grep 'api-explorer_0.6.0_linux_amd64.tar.gz' checksums.txt | sha256sum --check
```

macOS users can use:

```bash
grep 'api-explorer_0.6.0_darwin_arm64.tar.gz' checksums.txt | shasum -a 256 --check
```
...
```bash
tar -xzf api-explorer_0.6.0_linux_amd64.tar.gz
...
```bash
tar -xzf api-explorer_0.6.0_darwin_arm64.tar.gz
